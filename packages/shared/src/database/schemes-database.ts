 ...
 */
const DEFAULT_CONFIG: SchemesDatabaseConfig = {
 ...
 */
export class InMemorySchemesDatabase implements SchemesDatabase {
  private schemes: Map<string, GovernmentScheme> = new Map();
  private config: SchemesDatabaseConfig;
  ...
  updateScheme(id: string, updates: Partial<GovernmentScheme>): GovernmentScheme | null {
    ...
    const updated: GovernmentScheme = {
      ...existing,
      ...updates,
   ...
   */
  async getSchemesByCategory(category: BenefitType): Promise<GovernmentScheme[]> {
    return Array.from(this.schemes.values()).filter(
      scheme => scheme.isActive && (
        scheme.benefitType === category ||
  ...
  async getSchemesByState(state: string): Promise<GovernmentScheme[]> {
    ...
    return Array.from(this.schemes.values()).filter(scheme => {
      ...
      if (scheme.governmentLevel === 'central') return true;
   ...
   */
  async getStaleSchemes(thresholdDays?: number): Promise<GovernmentScheme[]> {
    const threshold = thresholdDays ?? this.config.stalenessThresholdDays;
    return Array.from(this.schemes.values()).filter(
      scheme => isSchemeStale(scheme, threshold)
    );
  }
  ...
  getStats(): {
    ...
    const staleSchemes = allSchemes.filter(s => 
      isSchemeStale(s, this.config.stalenessThresholdDays)
    );
