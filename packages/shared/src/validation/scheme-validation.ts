 ...
 */
const REQUIRED_SCHEME_FIELDS: (keyof GovernmentScheme)[] = [
  'id',
  'name',
  'description',
  'benefitType',
 ...
 */
export function validateCriterion(criterion: EligibilityCriterion): string[] {
  const errors: string[] = [];
 ...
 */
export function validateApplicationStep(step: ApplicationStep): string[] {
  const errors: string[] = [];
 ...
 */
export function validateDocument(doc: RequiredDocument): string[] {
  const errors: string[] = [];
 ...
 */
export function isSchemeStale(scheme: GovernmentScheme, thresholdDays: number): boolean {
  const now = new Date();
 ...
 */
export function getStaleSchemes(
 ...
 */
export function validateScheme(scheme: GovernmentScheme): SchemeValidationResult {
  const errors: string[] = [];
 ...
 */
export function hasCompleteSourceAttribution(scheme: GovernmentScheme): boolean {
  if (!scheme.source) return false;
